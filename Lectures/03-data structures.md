...
![[ds/Images/Pasted image 20250412211448.png|600]]
...
რუკა (Map)  ან ზოგ ენაში  ლექსიკონი (Dictionary)

---

![[Pasted image 20250424201003.png|800]]
...
![[ds/Images/Pasted image 20250412213911.png]]
...
![[ds/Images/Pasted image 20250411212803.png]]
...
![[ds/Images/Pasted image 20250411221137.png]]
...
---
რა არის ასიმპტოტური ანალიზი?
note:
ასიმპოტური ანალიზი არის მეთოდი რომლითაც ვიგებთ თუ ჩვენს ალგორითმს უფრო დიდი მოცულობის ინფორმაციას მივაწვდით რამდენად შენელდება ის.
ის არ გვიჩვენებს ზუსტ დროს მაგალითად 1 მილიწამი ან 5 წამი,ის უბრალოდ გვიჩვენებს როგორ იზრდება დრო როდესაც მონაცემთა ზომა იზრდება.ასემპტოტური ანალიზის გამოთვლის რამდენიმე ფუნქცია გვაქვს

---
ომეგა $\Omega()

---
რა არის ომეგა ფუნქცია და როდის გვჭირდება ის?
note:
ომეგა ფუნქცია გამოიყენება მაშინ როდესაც გვაინტერესებს საუკეთესო შემთხვევა ჩვენი ალგორითმის მუშაობის ანუ ყველაზე ცოტა მოქმედება რომელსაც შეასრულებს ჩვენი კოდი
...
note:
მაგალითში მოცემულია ჯავას კოდი რომელიც ეძებს რიცხვს მასივში.ამ შემთხვევაში შეგვიძლია ვთქვათ რომ აქ საუკეთესო შემთხვევა არის როდესაც საპოვნი ელემენტი უდრის პირველ ელემენტს ანუ ჩვენი ომეგა ფუნქციის ზრდა არ არის დამოკიდებული მასივის ზომაზე სხვაგვარად შეგვიძლია ვთქვათ რომ $\Omega(1)$
...
როდის ვიყენებთ O()-ს?
note:
განსხვავებით ომეგასგან დიდ ო ფუნქციას ვიყენებთ მაშინ როდესაც გვაინტერესებს ჩვენი კოდის მუშაობის ყველაზე ცუდი შემთხვევა
...
note:
განვიხილოთ იგივე მაგალითი.თუ წინა მაგალითში საუკეთესო შემთხვევა იყო როდესაც პირველი ელემენტი უდრიდა საპოვნ ელემენტს,ყველაზე ცუდი შემთხვევაა მაშინ როდესაც საძებნელი ელემენტი საერთოდ არ გვაქვს მასივში.ამ შემთხვევაში ჩვენი კოდში მოქმედებების რაოდენობა დამოკიდებულია მასივის სიგრძეზე ანუ ბიგ ო ფუნქცია წრფივი ფუნქციია ან სხვაგვარად რო ვთქვათ O(N)
...
---
```java
ppublic static void printAllPairs(int[] items) {
        int n = items.length;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
...
```
note:
განვიხილოთ სხვა მაგალითი. ამ მაგალითში ჩვენი კოდი ბეჭდავს მასივის ყველა შესაძლო წყვილს.ამ კოდის მოქმედებების რაოდენობა დამოკიდებულია N-ზე მაგრამ ამავდროულად გარე ციკლი შიდა ციკლს N-ჯერ ამეორებს ანუ საერთო მოქმედებების რაოდენობა n ჯერ ნ ის ტოლია რაც n^2ს უდრის.ანუ შეგვიძლია ვთქვათ რომ ამ შემთხვევაში ბიგ ო  ფუნქცია კვადრატულია ან უბრალოდ 
...
![[Pasted image 20250425210737.png]]
note:
ეს კი დიდი 0-ს ფუნქციის გრაფებია თვალნათლიბ წარმოდგენილი.ამაზე უფრო დეტალურად კი ნიკა მოგიყვებათ